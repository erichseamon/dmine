  ...
  setdir <- paste("/dmine/data/USDA/agmesh-scenarios/", scen_state, "/month_png/", dcause, sep="")
  listz <- list.dirs(setdir)
  liss <- length(listz)
  newframe <- t(data.frame(strsplit(listz, "\\/")[2:liss]))
  uni <- unique(newframe[,9])
  
  setwd(paste("/dmine/data/USDA/agmesh-scenarios/", scen_state, "/month_png/", sep=""))
  system(paste("mkdir ", "timelapse", sep=""))
  
  for (i in uni) {
    
  
  setwd(paste("/dmine/data/USDA/agmesh-scenarios/", scen_state, "/month_png/", dcause, "/", i, sep="")) 
