      ...
      for (jj in months_all) {
        ...
        if (nrow(DT2) != 0 ) {
          ...
          if (DTz != 0) {
            ...
            plot(m, col = newmatrix, main = paste(scen_state,  " ", jj, " ", j, " ", "\n", kkk, ": ", lll, sep=""), sub=paste("Total Monthly Loss: $", sum(m$loss), sep=""), col.sub="blue", cex.sub=2.5, cex.main = 2, line=1)
            ...
            par(adj = 0)
            bar <- barplot(nxx$loss, space = 0, col=cols, xlab="", ylab="", main = paste(scen_state,  " ", kkk, " for ", lll, "\n", " Total loss ($) by month, 1989 - 2015", sep="")
            ...
            if (nrow(DT2) == 0) {
              ...
              plot(m, col = newmatrix, main = paste(scen_state,  " ", jj, " ", j, " ", "\n", kkk,  ": ", lll, sep=""), sub=paste("\n Total Monthly Loss: $", sum(m$loss), sep=""), col.sub="blue", cex.sub=2.5, cex.main = 2, line=1)
