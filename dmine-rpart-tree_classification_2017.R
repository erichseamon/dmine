...
›rpart.plot(fit, type = 4, extra = 101, node.fun=node.fun)

library(partykit)
rparty.fit <- as.party(fit)
...
rfit <- randomForest(loss + tmmx + tmmn + srad + sph + vs + fm100 + erc + fm1000 + pdsi + pet + rmin + rmax,  data=data)
