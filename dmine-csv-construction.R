...
xx <- subset(combined_Allstates_monthlycount.df, loss != "NA")
xx <- subset(xx, loss != 0)

xx1 <- subset(combined_Allstates_monthlyloss.df, loss != "NA")
xx2 <- subset(xx1, loss != 0)

unique(xx$county)
xxx <- subset(xx, county == "Lincoln")
xxx

diff1 <- combined_Allstates_monthlyloss.df[setdiff(which(!!xx), which(!!xx2)),]

library(plyr)
arrange(xx2,desc(year),month)

x <- read.csv("/dmine/data/USDA/agmesh-scenarios/Allstates/summaries/Palouse_summary_sumloss.csv")
...
setwd("/dmine/data/USDA/agmesh-scenarios/Allstates/")
dirname <- "/dmine/data/USDA/agmesh-scenarios/Allstates/"
files <- list.files(dirname, pattern = '_summary', full.names=TRUE)
tables <- lapply(files, read.csv, header=TRUE)
combined_Allstates_sumloss.df <- do.call(rbind, tables)

#test number of counties

countiesX <- subset(combined_Allstates_sumloss.df, monthcode != 0)
