          ...
          lmat = NULL, lhei = NULL, lwid = NULL, extrafun = NULL, ...) 
{
  ...
  if (dendrogram %in% c("both", "row")) {
    ...
    if ("try-error" %in% class(flag)) {
      ...
      if (!is.null(cond) && conditionMessage(cond) == "evaluation nested too deeply: infinite recursion / options(expressions=)?") 
        stop("Row dendrogram too deeply nested, recursion limit exceeded.  Try increasing option(=...).")
    }
  }
  else plot.new()
  ...
  if (dendrogram %in% c("both", "column")) {
    ...
    if ("try-error" %in% class(flag)) {
      cond <- attr(flag, "condition")
      if (!is.null(cond) && conditionMessage(cond) == "evaluation nested too deeply: infinite recursion / options(expressions=)?") 
        stop("Column dendrogram too deeply nested, recursion limit exceeded.  Try increasing option(=...).")
    }
  }
  else plot.new()
  if (!is.null(main)) 
