getPET <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_pet_1979_CurrentYear_CONUS.nc', 
                    variables = "daily_mean_potential_evapotranspiration", 
  ...
  colnames(precipData_result_frame) <- c("daily_mean_potential_evapotranspiration")
  ...
  PETData <- merge(counties, precipData_result_frame, by="FIPS")
