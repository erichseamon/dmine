getCPCSOIL <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://www.esrl.noaa.gov/psd/thredds/dodsC/Datasets/cpcsoil/soilw.mon.mean.v2.nc', 
                    variables = "soilw", 
  ...
  colnames(precipData_result_frame) <- c("soilw")
  ...
  CPCSOILData <- merge(counties, precipData_result_frame, by="FIPS")
