getTMMN <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_tmmn_1979_CurrentYear_CONUS.nc', 
                    variables = "daily_minimum_temperature", 
  ...
  colnames(precipData_result_frame) <- c("daily_minimum_temperature")
  ...
  TMMNData <- merge(counties, precipData_result_frame, by="FIPS")
