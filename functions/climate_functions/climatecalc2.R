...
climatecalc <- function(statesclim, statevar, startdatec, enddatec) {
  ...
  statevar2 <- as.character(statevar)
  statevar2 <- substring(statevar2, 4, 7)
  
  
  final <- data.frame()
  for (i in 1:monthspan) {
    ii <- i+1
    precipData <- statevar(states = statesclim,
                            startDate = as.character(months[i]),
                            endDate = as.character(months[ii]))
