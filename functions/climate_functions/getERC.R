getERC <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_erc_1979_CurrentYear_CONUS.nc', 
                    variables = "daily_mean_energy_release_component-g", 
  ...
  colnames(precipData_result_frame) <- c("daily_mean_energy_release_component-g")
  ...
  ERCData <- merge(counties, precipData_result_frame, by="FIPS")
