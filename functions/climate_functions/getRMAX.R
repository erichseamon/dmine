getRMAX <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_rmax_1979_CurrentYear_CONUS.nc', 
                    variables = "daily_maximum_relative_humidity", 
  ...
  colnames(precipData_result_frame) <- c("daily_maximum_relative_humidity")
  ...
  RMAXData <- merge(counties, precipData_result_frame, by="FIPS")
