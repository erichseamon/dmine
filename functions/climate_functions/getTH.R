getTH <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_th_1979_CurrentYear_CONUS.nc', 
                    variables = "daily_mean_wind_direction", 
  ...
  colnames(precipData_result_frame) <- c("daily_mean_wind_direction")
  ...
  THData <- merge(counties, precipData_result_frame, by="FIPS")
