getFM100 <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_fm100_1979_CurrentYear_CONUS.nc', 
                    variables = "dead_fuel_moisture_100hr", 
  ...
  colnames(precipData_result_frame) <- c("dead_fuel_moisture_100hr")
  ...
  FM100Data <- merge(counties, precipData_result_frame, by="FIPS")
