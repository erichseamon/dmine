getBI <- function(states, startDate, endDate){
  ...
  fabric <- webdata(url = 'http://thredds.northwestknowledge.net:8080/thredds/dodsC/agg_met_bi_1979_CurrentYear_CONUS.nc', 
                    variables = "daily_mean_burning_index_g", 
  ...
  colnames(precipData_result_frame) <- c("daily_mean_burning_index_g")
  ...
  BIData <- merge(counties, precipData_result_frame, by="FIPS")
