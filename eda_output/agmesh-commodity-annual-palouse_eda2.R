...
pairs(countratio ~ pr + pdsi + pet + erc + tmmx + soil_moisture_shorterm + soil_moisture_longterm, lower.panel=panel.smooth, upper.panel=panel.cor, data=myfiles_f, main="1989-2015 WHEAT Drought Whitman County, Countratio")
...
#--some linear models
dev.off()
plot(lm(loss ~ pr+pdsi+pet+tmmx+soil_moisture_shorterm*year, data=myfiles_f))

dev.off()
layout(matrix(c(1,2,3,4,5,6),3,2)) # optional 4 graphs/page 

lmcount <- lm(loss ~ pr+pdsi+pet+tmmx+soil_moisture_shorterm*year, data=myfiles_f)
...
fit08_VIF <- VIF(lm(loss ~ pr + pet + prpet + pdsi + tmmx, data=myfiles_f))
